...
describe("pre-req-vault", () => {
  ...
  it(" Withdraw 0.5 Sol from the vault", async () => {
    ...
    const githubOffset = 8 + 32 + 1 + 1 + 1;
    const githubLen = registrationAccount!.data.readUInt32LE(githubOffset);
    const github = registrationAccount!.data
      .slice(githubOffset + 4, githubOffset + 4 + githubLen)
      .toString();
    expect(github).to.equal("akshitj11");
  });
